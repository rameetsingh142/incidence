...
def launch_gui():
    ...
    # --- Loader/Please Wait Label ---
    loader_label = tk.Label(root, text="Please wait, loading and analyzing data...", font=("Arial", 16, "bold"), fg="#1976d2", bg="#f0f4fa")
    loader_label.place(relx=0.5, rely=0.5, anchor="center")
    loader_label.lower()  # Hide initially
    ...
    def run_search(medical_condition, output_widget, progress_bar):
        ...
        # --- INCIDENCE/PREVALENCE/MORTALITY SECTION ---
        output_widget.insert(tk.END, "==== INCIDENCE / PREVALENCE / MORTALITY DATA ====\n", "incidence_highlight")
        ...
        # Increase number of queries and results for deeper search
        query4 = f"{medical_condition} epidemiology India"
        query5 = f"{medical_condition} annual cases India"
        query6 = f"{medical_condition} burden India"
        query7 = f"{medical_condition} india research paper"
        search_queries = [query1, query2, query3, query4, query5, query6, query7]

        all_urls_found = []
        for query in search_queries:
            try:
                for url in search(query, num_results=8, lang="en"):
                    if url not in all_urls_found:
                        all_urls_found.append(url)
                    if len(all_urls_found) >= 16:
                        break
                if len(all_urls_found) >= 16:
                    break
                time.sleep(0.5)
        ...
        if not all_urls_found:
            ...
            urls_to_process = all_urls_found[:8]
            progress_bar["maximum"] = min(8, len(urls_to_process))
            processed_urls_count = 0

            for url in urls_to_process:
                if processed_urls_count >= 8:
                    break
                root.update_idletasks()
        ...
        output_widget.insert(tk.END, "==== INDIAN MARKET DATA ====\n", "incidence_highlight")

        # Market-specific queries, highly restrictive to Indian market and deep search
        market_queries = [
            f"{medical_condition} indian market size inr usd crore billion lakh",
            f"{medical_condition} india market value report government publication",
            f"{medical_condition} india market research paper journal pubmed",
            f"{medical_condition} india industry revenue statistics ministry",
            f"{medical_condition} india market forecast CAGR trends",
            f"{medical_condition} india healthcare market size",
            f"{medical_condition} india pharmaceutical market report",
            f"{medical_condition} india market analysis official data",
            f"{medical_condition} india market opportunity future growth",
            f"{medical_condition} india market government data publication"
        ]
        ...
        for query in market_queries:
            try:
                for url in search(query, num_results=8, lang="en"):
                    if url not in market_urls_found:
                        market_urls_found.append(url)
                    if len(market_urls_found) >= 16:
                        break
                if len(market_urls_found) >= 16:
                    break
                time.sleep(0.5)
            except Exception as e:
                output_widget.insert(tk.END, f"Error during market search: {e}\n", "incidence_highlight")
                output_widget.update()
                continue

        if not market_urls_found:
            output_widget.insert(tk.END, "No relevant market URLs found.\n", "incidence_highlight")
            progress_bar["value"] = 0
            return

        urls_to_process = market_urls_found[:8]
        progress_bar["maximum"] = min(8, len(urls_to_process))
        processed_urls_count = 0

        # Reduce number of processed URLs for market section to speed up
        max_market_links = 4
        urls_to_process = urls_to_process[:max_market_links]
        ...
        for url in urls_to_process:
            ...
            # Restrictive Indian market keywords + research/gov/publication
            market_keywords = [
                "india", "indian market", "market size", "market value", "market share", "market forecast", "market growth",
                "industry size", "industry value", "industry revenue", "market inr", "market usd",
                "billion", "million", "crore", "lakh", "future growth", "cagr", "market opportunity",
                "market analysis", "market report", "market trends", "market research",
                "government", "govt", "ministry", "report", "publication", "journal", "doi", "pubmed", "study", "research",
                "official", "statistics", "healthcare", "pharmaceutical"
            ]
            sentences = re.split(r'(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?|\!)\s', page_text[:4000])
            ...
            for sentence in sentences:
                s = sentence.strip()
                if (
                    ("india" in s or "indian" in s) and
                    any(kw in s for kw in market_keywords) and
                    re.search(r'\b\d[\d,\.]*\s*(billion|million|crore|lakh|usd|inr|rs|₹|\$)', s)
                ):
                    output_widget.insert(tk.END, s + "\n", "incidence_highlight")
                    found_market_data = True
            if not found_market_data:
                output_widget.insert(tk.END, "  (No clear Indian market data with numbers found in this link)\n", "incidence_highlight")
            output_widget.update()
            root.update_idletasks()
            time.sleep(0.01)
